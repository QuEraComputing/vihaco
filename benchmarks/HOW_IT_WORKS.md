...
its own unpublished Cargo workspace and Python runner.
...
insufficient to reproduce them.
...
startup. Subprocess timeouts limit how long a run can continue; their values are
defined in the runner.
...
place; redirected output receives periodic plain-text updates. The parent
...
metadata, commands, calibration diagnostics, and raw logs. The rest of the run
directory, including failure diagnostics, stays on the machine that ran the
benchmarks.
...
still needs an environment without secrets and a checkout without credentials.
...
identify changes worth investigating and do not affect whether a quality check
passes.
...
- `checks.py`: quality checks with local-only diagnostics.
...
the development tools. The quality checks cover types and Ruff lint/formatting;
formatting is limited to `runner/` and `tests/python/` to leave measured
