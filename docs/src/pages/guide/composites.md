...
```rust ignore
# use eyre::Result;
# use vihaco::{dispatch, Component, Effects, Instruction, Parse, observe};
...
`<Field as Component>::Runtime` and `<Field as Component>::Syntax` types.

The syntax parser delegates to each component's `Syntax::parser()`. Component
...
The field type must implement `Component` (which `component!` provides). The device code and aliases are what a loader uses to validate source symbols and route instructions when a composite loads a module.
