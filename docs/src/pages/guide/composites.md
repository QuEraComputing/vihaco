...
```rust ignore
# use eyre::Result;
# use vihaco::{dispatch, Component, Effects, HasInstructionSet, Instruction, Parse, observe};
# #[derive(Debug, Clone, Instruction)]
# pub enum CounterInst { Print }
#[derive(Debug, Clone, PartialEq, Parse)]
#[syntax_class(instruction)]
enum CounterSyntax { #[pattern = "'print"] Print }
# #[derive(Debug, Default)]
# pub struct Counter;
...
- **Composite traits** — the annotated struct implements `Composite` and
  `HasInstructionSet`. Its `Runtime` and `Syntax` instruction types are the
  generated `composite_module::runtime::Instruction` and
  `composite_module::syntax::Instruction` enums described below; no manual
  `HasInstructionSet` implementation is needed for the composite itself.
...
- **Runtime and syntax instruction enums** in a snake_case module named after the composite. Each has one variant per device field. For `CounterComposite` above, the runtime module contains an enum shaped roughly like:
  ...
  pub enum Instruction {
      Counter(<Counter as HasInstructionSet>::Runtime),
  }
  ```
...
It also generates a snake_case module analogous to the module generated by
`component!`. For `CounterComposite`, the module contains
`counter_composite::runtime::Instruction` and
`counter_composite::syntax::Instruction`. Both enums have one PascalCase
variant per device field; their payloads are respectively that field's
`<Field as HasInstructionSet>::Runtime` and `<Field as HasInstructionSet>::Syntax` types.

The syntax parser delegates to each component's device's
`<Device as HasInstructionSet>::Syntax::parser()`. Component
syntax generated by `component!` uses the component's snake_case name as its
dialect head. Thus `Counter` accepts `counter.halt`. The composite then
attaches the device field name as an outer instruction head, so a `counter`
field accepts `counter::counter.halt` and produces the composite instruction
variant `Counter(...)`.

Device aliases can replace the field-name head, so the alias `count` also
accepts `count::counter.halt`. The field name and aliases are alternative
outer heads; the component's dialect and instruction mnemonic are supplied by
its own syntax parser.
...
The field type must implement `HasInstructionSet` (which `component!` provides). Device fields that execute instructions should also implement `Component`; `#[composite]` provides the `Composite` and `HasInstructionSet` implementations for the composite itself. The device code and aliases are what a loader uses to validate source symbols and route instructions when a composite loads a module.
