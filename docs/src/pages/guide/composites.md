...
The syntax parser delegates to each component's `Syntax::parser()`. Since a
component parser already consumes its component head, the composite parser
consumes the device name first. Thus a component whose own syntax is
`counter::halt` is parsed by the composite as `counter::counter::halt`.
Device aliases can replace the first prefix, so the alias `count` accepts
`count::counter::halt`.
