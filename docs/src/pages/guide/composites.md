...
The syntax parser delegates to each device's
`<Device as HasInstructionSet>::Syntax::parser()`. Component
syntax generated by `component!` is intentionally headless: it parses the
instruction mnemonic, such as `halt`, without adding a namespace. The
composite then attaches the device field name as the instruction head. Thus a
`counter` field accepts `counter::halt` and produces the composite instruction
variant `Counter(...)`.

Device aliases can replace the field-name head, so the alias `count` also
accepts `count::halt`. The field name and aliases are alternative heads; the
component's instruction mnemonic is supplied by its own syntax parser.
