...
use vihaco_parser::{Ident, Parse as ParseTrait, SurfaceInstruction};
...
impl<'src> ParseTrait<'src> for Operand {
    ...
    {
        vihaco_parser::ident().map(Operand)
    }
}
...
#[syntax_class(instruction)]
enum DigitalDialect {
    #[pattern = "'digital::set $0"]
    Set(i64),
}

#[test]
fn complete_instruction_tokens_select_the_dialect() {
...
fn generics_and_generated_lifetime_name_collisions_compile_and_parse() {
    ...
    assert_eq!(parse("value 41"), Ok(GenericInstruction::Value(41_i64)));
