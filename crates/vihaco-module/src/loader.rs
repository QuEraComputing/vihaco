...
/// Allow a machine to load the SST program owned directly by its section.
///
...
/// make its own program behavior explicit, even when that behavior is a no-op.
pub trait LoadSstProgram<C> {
    fn load_sst_program<'bc>(&mut self, section: SstSectionView<'bc, C>) -> eyre::Result<()>;
}

/// Allow a machine to load an SST subtree completely.
///
/// For composites generated by [`vihaco_runtime_derive::composite`], this loads the
/// composite's own program through [`LoadSstProgram`] and then forwards
/// direct child sections to `#[loadable]` devices.
pub trait LoadSstSubtree<C> {
    fn load_sst_subtree<'bc>(&mut self, section: SstSectionView<'bc, C>) -> eyre::Result<()>;
}
